...
const pitchTypeLowerBetter = {
  batter: [],
  pitcher: ["homeRuns", "woba", "est_woba", "ba", "slg", "est_ba", "est_slg", "hard_hit_percent"]
};
...
function formatPitchTypeStat(key, value) {
  ...
  if (["pitches", "pa", "homeRuns"].includes(key)) return Math.round(number).toLocaleString("en-US");
...
function aggregatePitchTypeItems(items, groupKey) {
  ...
  return {
    ...
    homeRuns: sumRows(selected, "homeRuns"),
...
function aggregatePitchTypeRows() {
  ...
  pitchTypeRows = Array.from(selectedGroups.entries()).map(([key, items]) => {
    ...
    return {
      ...
      pitch_usage: totalPitches ? pitches / totalPitches * 100 : null,
      pa: sumRows(items, "pa"),
      homeRuns: sumRows(items, "homeRuns"),
        ...
        <tbody>
          ${rows.map((row) => `
            <tr>
              ...
              <td>${formatPitchTypeStat("homeRuns", row.homeRuns)}</td>
...
function renderPitchTypeTableHead() {
  const columns = [
    ...
    ["homeRuns", pitchTypeSide === "batter" ? "HR" : "HR Allowed"],
...
function renderPitchTypeTable() {
  ...
  `).join("") || `<tr><td colspan="15" class="empty-row">No rows match this filter.</td></tr>`;
}
...
async function loadPitchTypeData() {
  ...
  try {
    ...
    document.querySelector("#pitch-types-table").innerHTML = `<tr><td colspan="15" class="empty-row">Baseball Savant did not return pitch type data. Refresh the page or try another season.</td></tr>`;
  }
}
